...
/**
 * Manages a pool of prewarmed (eager start) resources for an AppDefinition.
 * 
 * Responsibilities:
 * - Creating/scaling the pool of prewarmed instances
 * - Reserving instances for sessions
 * - Releasing instances back to the pool
...
public class PrewarmedResourcePool {
    ...
    /**
     * Ensures the pool has the specified minimum number of instances.
     * Creates missing resources (services, configmaps, deployments).
     */
    public boolean ensureCapacity(AppDefinition appDef, int minInstances, String correlationId) {
        LOGGER.info(formatLogMessage(correlationId, "Ensuring pool capacity: " + minInstances + " for " + appDef.getSpec().getName()));
        ...
        List<Service> existingServices = K8sUtil.getExistingServices(
                client.kubernetes(), client.namespace(), ownerName, ownerUID);
        ...
        // Compute missing IDs
        Set<Integer> missingServiceIds = TheiaCloudServiceUtil.computeIdsOfMissingServices(
                appDef, correlationId, minInstances, existingServices);
        Set<Integer> missingDeploymentIds = TheiaCloudDeploymentUtil.computeIdsOfMissingDeployments(
                appDef, correlationId, minInstances, existingDeployments);

        boolean success = true;

        // Create missing services
        for (int instance : missingServiceIds) {
            success &= resourceFactory.createServiceForEagerInstance(appDef, instance, labels, correlationId).isPresent();
            success &= resourceFactory.createInternalServiceForEagerInstance(appDef, instance, labels, correlationId).isPresent();
        }
        ...
        if (arguments.isUseKeycloak()) {
            ...
            Set<Integer> missingProxyIds = TheiaCloudConfigMapUtil.computeIdsOfMissingProxyConfigMaps(
                    appDef, correlationId, minInstances, proxyConfigMaps);
            Set<Integer> missingEmailIds = TheiaCloudConfigMapUtil.computeIdsOfMissingEmailConfigMaps(
                    appDef, correlationId, minInstances, emailConfigMaps);

            for (int instance : missingProxyIds) {
                success &= resourceFactory.createProxyConfigMapForEagerInstance(appDef, instance, labels, correlationId).isPresent();
            }
            for (int instance : missingEmailIds) {
                success &= resourceFactory.createEmailConfigMapForEagerInstance(appDef, instance, labels, correlationId).isPresent();
            }
        }

        // Create missing deployments
        for (int instance : missingDeploymentIds) {
            success &= resourceFactory.createDeploymentForEagerInstance(appDef, instance, labels, correlationId).isPresent();
        }
    ...
    /**
     * Reconciles the pool to match the target instance count.
     * Creates missing instances, removes excess instances (respecting ownership).
     */
    public boolean reconcile(AppDefinition appDef, int targetInstances, String correlationId) {
        ...
        List<Service> externalServices = existingServices.stream()
                .filter(s -> !s.getMetadata().getName().endsWith("-int"))
                .collect(Collectors.toList());
        Set<Integer> missingServiceIds = TheiaCloudServiceUtil.computeIdsOfMissingServices(
                appDef, correlationId, targetInstances, existingServices);

        success &= ResourceLifecycleManager.reconcile(
                ResourceLifecycleManager.ReconcileContext.<Service>builder()
                        .correlationId(correlationId)
                        .existingResources(externalServices)
                        .missingIds(missingServiceIds)
                        ...
                        .shouldRecreate(s -> OwnershipManager.isOwnedSolelyBy(s, owner))
                        .recreateResource(s -> {
        ...
        if (arguments.isUseKeycloak()) {
            ...
            Set<Integer> missingProxyIds = TheiaCloudConfigMapUtil.computeIdsOfMissingProxyConfigMaps(
                    appDef, correlationId, targetInstances, proxyConfigMaps);
            Set<Integer> missingEmailIds = TheiaCloudConfigMapUtil.computeIdsOfMissingEmailConfigMaps(
                    appDef, correlationId, targetInstances, emailConfigMaps);

            success &= ResourceLifecycleManager.reconcile(
                    ResourceLifecycleManager.ReconcileContext.<ConfigMap>builder()
                            .correlationId(correlationId)
                            .existingResources(proxyConfigMaps)
                            .missingIds(missingProxyIds)
                            .targetCount(targetInstances)
                            .owner(owner)
                            .resourceAccessor(cm -> client.kubernetes().configMaps().inNamespace(client.namespace()).resource(cm))
                            .idExtractor(cm -> TheiaCloudConfigMapUtil.getProxyId(correlationId, appDef, cm))
                            .resourceTypeName("proxy configmap")
                            .createResource(instance -> resourceFactory.createProxyConfigMapForEagerInstance(appDef, instance, labels, correlationId))
            ...
            success &= ResourceLifecycleManager.reconcile(
                    ResourceLifecycleManager.ReconcileContext.<ConfigMap>builder()
                            .correlationId(correlationId)
                            .existingResources(emailConfigMaps)
                            .missingIds(missingEmailIds)
                            .targetCount(targetInstances)
                            .owner(owner)
                            .resourceAccessor(cm -> client.kubernetes().configMaps().inNamespace(client.namespace()).resource(cm))
                            .idExtractor(cm -> TheiaCloudConfigMapUtil.getEmailId(correlationId, appDef, cm))
                            .resourceTypeName("email configmap")
                            .createResource(instance -> resourceFactory.createEmailConfigMapForEagerInstance(appDef, instance, labels, correlationId))
                            .shouldRecreate(cm -> OwnershipManager.isOwnedSolelyBy(cm, owner))
                            .recreateResource(cm -> {
        ...
        List<Deployment> existingDeployments = K8sUtil.getExistingDeployments(
                client.kubernetes(), client.namespace(), ownerName, ownerUID);
        Set<Integer> missingDeploymentIds = TheiaCloudDeploymentUtil.computeIdsOfMissingDeployments(
                appDef, correlationId, targetInstances, existingDeployments);

        success &= ResourceLifecycleManager.reconcile(
                ResourceLifecycleManager.ReconcileContext.<Deployment>builder()
                        .correlationId(correlationId)
                        .existingResources(existingDeployments)
                        .missingIds(missingDeploymentIds)
                        .targetCount(targetInstances)
                        .owner(owner)
                        .resourceAccessor(d -> client.kubernetes().apps().deployments().inNamespace(client.namespace()).resource(d))
                        .idExtractor(d -> TheiaCloudDeploymentUtil.getId(correlationId, appDef, d))
                        .resourceTypeName("deployment")
                        .createResource(instance -> resourceFactory.createDeploymentForEagerInstance(appDef, instance, labels, correlationId))
                        .shouldRecreate(d -> OwnershipManager.isOwnedSolelyBy(d, owner))
                        .recreateResource(d -> {
    ...
    public boolean releaseAll(AppDefinition appDef, String correlationId) {
        ...
        List<Service> services = K8sUtil.getExistingServices(client.kubernetes(), client.namespace(), ownerName, ownerUID);
        ...
        List<ConfigMap> configMaps = K8sUtil.getExistingConfigMaps(client.kubernetes(), client.namespace(), ownerName, ownerUID);
        success &= ResourceLifecycleManager.releaseOwnership(configMaps, owner,
                ...
                "configmap", correlationId);

        List<Deployment> deployments = K8sUtil.getExistingDeployments(client.kubernetes(), client.namespace(), ownerName, ownerUID);
    ...
    /**
     * Reserves a prewarmed instance for a session.
     * Adds the session as an owner of the instance's resources.
     */
    public synchronized ReservationResult reserveInstance(Session session, AppDefinition appDef, String correlationId) {
        LOGGER.info(formatLogMessage(correlationId, "Attempting to reserve instance for session " + session.getMetadata().getName()));
        ...
        List<Service> existingServices = K8sUtil.getExistingServices(
                client.kubernetes(), client.namespace(), appDefOwnerName, appDefOwnerUID);
        ...
        Optional<Service> alreadyReservedExternal = TheiaCloudServiceUtil.getServiceOwnedBySession(
                sessionName, sessionUID, externalServices);
        Optional<Service> alreadyReservedInternal = TheiaCloudServiceUtil.getServiceOwnedBySession(
                sessionName, sessionUID, internalServices);
        ...
        if (alreadyReservedExternal.isPresent() ^ alreadyReservedInternal.isPresent()) {
            return handlePartialReservation(session, appDef,
                    alreadyReservedExternal, alreadyReservedInternal,
        ...
        return ReservationResult.success(new PoolInstance(chosenInstance, chosenExternal, chosenInternal, deploymentName));
    }

    /**
     * Completes the session setup after reservation.
     * Adds session labels, reserves deployment, configures email config.
     */
    public boolean completeSessionSetup(
            Session session,
        ...
        // Reserve deployment
        try {
            client.kubernetes().apps().deployments().withName(instance.getDeploymentName())
                    .edit(d -> TheiaCloudHandlerUtil.addOwnerReferenceToItem(correlationId, sessionName, sessionUID, d));
        } catch (KubernetesClientException e) {
        ...
        if (arguments.isUseKeycloak()) {
            ...
            try {
                client.kubernetes().configMaps().withName(emailConfigName)
                        .edit(cm -> {
                            cm.setData(Collections.singletonMap(
    ...
    /**
     * Releases an instance back to the pool (used when session ends).
     * Removes session ownership and clears session-specific data.
     */
    public boolean releaseInstance(Session session, AppDefinition appDef, String correlationId) {
        LOGGER.info(formatLogMessage(correlationId, "Releasing instance for session " + session.getMetadata().getName()));
        ...
        if (services.isEmpty()) {
            LOGGER.error(formatLogMessage(correlationId, "No services found for session " + session.getSpec().getName()));
            return false;
        }
        ...
        try {
            client.kubernetes().apps().deployments().withName(deploymentName)
                    .edit(d -> TheiaCloudHandlerUtil.removeOwnerReferenceFromItem(correlationId, sessionName, sessionUID, d));
        } catch (KubernetesClientException e) {
        ...
        if (arguments.isUseKeycloak()) {
            ...
            try {
                client.kubernetes().configMaps().withName(emailConfigName)
                        .edit(cm -> {
                            cm.setData(Collections.singletonMap(
                                    AddedHandlerUtil.FILENAME_AUTHENTICATED_EMAILS_LIST, null));
                            return cm;
                        });
            } catch (KubernetesClientException e) {
    ...
    private ReservationResult handlePartialReservation(
            Session session, AppDefinition appDef,
            Optional<Service> reservedExternal, Optional<Service> reservedInternal,
            Map<Integer, Service> externalMap, Map<Integer, Service> internalMap,
            String correlationId) {
    ...
    private void rollbackReservation(Service service, String sessionName, String sessionUID, String correlationId) {
        try {
            client.services().withName(service.getMetadata().getName())
                    .edit(s -> {
    ...
    private List<Service> findServicesByLabels(Map<String, String> labels, String correlationId) {
        ...
        return client.kubernetes().services()
